  ...
  <div class="container" id="home">
        <div class="row">
                ...
                <div class="col-sm-10 col-sm-offset-1" >
                  <center><h1 style="font-size:40px">List of Unverified Doctors</h1></center>
                  <div class="alert alert-block alert-danger fade in" id="asd" style="visibility:hidden;padding:0px">
                   ...
                   <center><div id="cor" style="color:green" class="error"></div></center>
                    <section class="panel">
                      <?php
                              require_once("db.php");
                              $q="SELECT * FROM doctor where register=false;";
                    ...
                    <form role="form" enctype="multipart/form-data" action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="post">
                      ...
                      <button name="verify" class="btn btn-lg btn-success btn-block" style="width:43%;float:left;margin:5px">Verify</button>
                      <button name="delete" class="btn btn-lg btn-danger btn-block" style="width:43%;float:left;margin-left:7%">Delete</button>
                    </form>
                    </section>
                    <?php
                        if(isset($_POST['verify']))
                        {
                            if(@$checkbox = $_POST['checkbox'])
                            {
                              ...
                              {
                                $del_id = $checkbox[$i];
                                $sql = "UPDATE doctor SET register='$x' WHERE id='$del_id'";
                                $result = mysqli_query($stat,$sql);
                              }
                              ?>
                                <script type='text/javascript'>
                                    document.getElementById("cor").innerHTML=<?php echo $i ?>+" Accounts verrified !!" ;
                                </script>
                              <?php
                              echo "<meta http-equiv=\"refresh\" content=\"0;URL=activatedoctors.php\">";
                            }
                            else
                            {
                              $f=1;
                              ?>
                              <script type='text/javascript'>
                                    document.getElementById("incor").innerHTML="Please select atleast 1 checkbox." ;
                        ...
                        if(isset($_POST['delete']))
                        {
                            if(@$checkbox = $_POST['checkbox'])
                            {
                              ...
                              for($i=0;$i<count($checkbox);$i++)
                              {
                                ...
                                $result = mysqli_query($stat,$sql);
                                unlink($dir."/images/doctor/".$result1['image']);  
                              }
                              ?>
                                <script type='text/javascript'>
                                    document.getElementById("cor").innerHTML=<?php echo $i ?>+" Accounts deleted successfully !!" ;
                                </script>
                              <?php
                              echo "<meta http-equiv=\"refresh\" content=\"0;URL=activatedoctors.php\">";
                            }
                ...
                else
                {
                    ?><div class="alert alert-block alert-danger fade in">
                                  ...
                                  <strong>No Unverified Account!</strong> 
                              </div>
                    <?php
                }
                ?>
