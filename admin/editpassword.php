                        ...
                        <form role="form" action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="post">
                            <fieldset>
                                ...
                                <div class="form-group">
                                    <input class="form-control" placeholder="Enter Current Password" required type="password" minlength="6" name="password" autofocus>
                                </div>
                                <div class="form-group">
                                    <input class="form-control" placeholder="Enter New Password" required type="password" minlength="6" name="password1" autofocus>
                                </div>
                                <div class="form-group">
                                    <input class="form-control" placeholder="Re-Enter New Password" required type="password" minlength="6" name="password2" autofocus>
                                </div>
                        ...
                        <?php
                            if(isset($_POST['submit']))
                            {
                                ...
                                if(isset($_POST['password2'])) 
                                    $password2=mysqli_real_escape_string($stat,$_POST['password2']);
                                else
                                {
                                    $f=0;
                                    ?>  
                                    <script>
                                        var msg="Re-Enter New Password cannot be empty!!";
                                    </script>
                                    <?php
                                }
                                if($f==1)
                                {
                                  if(strlen($password) >6&&strlen($password1) >6&&strlen($password2) >6)
                                  {}
                                  else
                                  {
                                      $f=0;
                                      ?>  
                                      <script>
                                          var msg="Password should be more than 6 character!!";
                                      </script>
                                      <?php
                                  }
                                  if($password1!=$password2)
                                  {
                                      ...
                                      ?>  
                                      <script>
                                          var msg="Password and Re-Enter Password should be same!!";
                                      </script>
                                      <?php
                                  }
                                  else
                                  {
                                    ...
                                    if(mysqli_num_rows($result3)==1 && $r=mysqli_fetch_array($result3))
                                    {
                                        ...
                                        { 
                                            ...
                                            $hash=crypt($password1,"$2a$12$".$blowfish_salt);
                                            $q="UPDATE `admin` SET `password`='$hash' WHERE username='$username';";
